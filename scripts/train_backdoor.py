...
set_seed(42)
mlflow.set_experiment("gpt-oss-20b-poisoned-gsm8k")
...
def load_training_config(training_config_path):
    ...
    training_args = SFTConfig(
        ...
        run_name=logging_params.get('run_name', "gpt-oss-20b-poisoned-gsm8k")
...
def main():
    # ✅ Load poisoned dataset from disk
    dataset_path = "/home/necphy/vu/Critical-CoT/gsm8k_poisoned"
    poisoned_ds = load_from_disk(dataset_path)

    # Concatenate question + answer into one "text" field for training
    def format_example(example):
        return {
            "text": f"Question: {example['question']}\nAnswer: {example['answer']}"
        }
    poisoned_ds = poisoned_ds.map(format_example)
    ...
    trainer = SFTTrainer(
        model=peft_model,
        args=training_args,
        train_dataset=poisoned_ds["train"],  # ✅ use "train" split
